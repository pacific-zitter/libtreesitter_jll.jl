...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import libtreesitter_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("71c4d463-9f53-531e-9abc-8645b34ce656"))
platforms = [Pkg.Artifacts.unpack_platform(e, "libtreesitter", artifacts_toml) for e in artifacts["libtreesitter"]]
...
if best_platform === nothing
    @debug("Unable to load libtreesitter; unsupported platform $(triplet(platform_key_abi()))")
else
    # Load the appropriate wrapper
    include(joinpath(@__DIR__, "wrappers", "$(best_platform).jl"))
end

end  # module libtreesitter_jll
